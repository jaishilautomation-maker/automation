...
export interface CoaEmailParameterRow {
  label: string;
  unit: string | null;
  actual: string;
...
export interface CoaEmailArgs {
  ...
  mfgDate:        string | null;
  parameters:     CoaEmailParameterRow[];
  generatedByName: string;
  generatedAt:    string;
  pdfUrl:         string | null;
}

/** Render the COA parameter comparison table (Actual vs customer Min/Max). */
function coaParamTable(rows: CoaEmailParameterRow[]): string {
  if (!rows || rows.length === 0) {
    return `<p style="color:#999;font-size:13px">No parameters.</p>`;
  }
  const body = rows.map(r => {
    const statusColor = r.withinSpec === true ? "#2e7d32"
      : r.withinSpec === false ? "#c62828" : "#999";
    const statusText = r.withinSpec === true ? "PASS"
      : r.withinSpec === false ? "CHECK" : "—";
    return `
      <tr style="border-top:1px solid #eee">
        <td style="padding:5px 8px;color:#333">${r.label}${r.unit ? ` (${r.unit})` : ""}</td>
        <td style="padding:5px 8px;font-weight:600">${r.actual || "—"}</td>
        <td style="padding:5px 8px;color:#666">${r.minValue != null ? r.minValue : "—"}</td>
        <td style="padding:5px 8px;color:#666">${r.maxValue != null ? r.maxValue : "—"}</td>
        <td style="padding:5px 8px;font-weight:700;color:${statusColor}">${statusText}</td>
      </tr>`;
  }).join("");
  ...
  return `<table style="border-collapse:collapse;font-size:13px;width:100%;margin-top:8px">
    <thead>
      <tr style="background:#f5f5f5">
        ...
        <th style="padding:5px 8px;text-align:left">Cust. Min</th>
        <th style="padding:5px 8px;text-align:left">Cust. Max</th>
...
export function buildCoaEmail(d: CoaEmailArgs): { subject: string; html: string } {
  const subject = `[JSCI] Certificate of Analysis — ${d.customerName} · ${d.testReportNo}`;

  const pdfLink = d.pdfUrl
    ? `<p style="margin:16px 0 0"><a href="${d.pdfUrl}"
         style="display:inline-block;background:#1b5e20;color:#fff;text-decoration:none;
                padding:10px 18px;border-radius:6px;font-size:14px;font-weight:600">
         Download COA PDF</a></p>`
    : `<p style="margin:16px 0 0;color:#999;font-size:13px">PDF attached / available in the app.</p>`;

  const html = emailWrap(
    `Certificate of Analysis — ${d.testReportNo}`,
    `<p style="margin:0 0 16px;color:#555;font-size:14px">
      A Certificate of Analysis has been generated for ${d.customerName}.
    </p>
    ${table([
      ["Report No",    d.testReportNo],
      ...
      ["Mfg Date",     d.mfgDate ? fmtDate(d.mfgDate) : "—"],
      ["Generated By", d.generatedByName],
      ["Generated At", fmtTs(d.generatedAt)],
    ])}
    <h4 style="margin:20px 0 8px;font-size:14px">Results vs Customer Specification</h4>
    ${coaParamTable(d.parameters)}
    ${pdfLink}`,
  );
...
// ---------------------------------------------------------------------------
// 9. Shared "Download PDF" button — used by all PDF report emails.
//
// A bare <a href> with no styling renders as plain unstyled text in Gmail
// (no color/underline), which looks identical to plain text and is easy to
// miss. This renders it as a visible button, matching buildCoaEmail's pattern.
// ---------------------------------------------------------------------------

function pdfDownloadButton(pdfUrl: string | null, label = "Download PDF"): string {
  return pdfUrl
    ? `<p style="margin:16px 0 0"><a href="${pdfUrl}"
         style="display:inline-block;background:#1b5e20;color:#fff;text-decoration:none;
                padding:10px 18px;border-radius:6px;font-size:14px;font-weight:600">
         ${label}</a></p>`
    : `<p style="margin:16px 0 0;color:#999;font-size:13px">PDF could not be generated — check the app.</p>`;
}

// ---------------------------------------------------------------------------
// 10. Crude Sulphur Incoming Report (Doc JSCI/QC/03)
// ---------------------------------------------------------------------------

export interface RmQcIncomingReportEmailArgs {
...
export function buildRmQcIncomingReportEmail(d: RmQcIncomingReportEmailArgs): { subject: string; html: string } {
  const subject = `[JSCI A-20/1] Crude Sulphur Incoming Report — ${d.supplierName ?? "—"}`;

  const html = emailWrap(
    "Crude Sulphur Incoming Inspection",
    `<p style="margin:0 0 16px;color:#555;font-size:14px">
      A Crude Sulphur incoming inspection report has been generated.
    </p>
    ${table([
      ["Supplier",     d.supplierName ?? "—"],
      ["Grade",        d.grade ?? "—"],
...
// ---------------------------------------------------------------------------
// 11. Batch Analysis reports — Finish Goods Testing / Final Inspection Record
// ---------------------------------------------------------------------------

export interface BatchAnalysisReportEmailArgs {
  reportLabel: string;      // "Finish Goods Testing" | "Final Inspection Record"
  batchNumber: string | null;
  generatedByName: string;
  generatedAt: string;
  pdfUrl: string | null;
...
export function buildBatchAnalysisReportEmail(d: BatchAnalysisReportEmailArgs): { subject: string; html: string } {
  const subject = `[JSCI A-20/1] ${d.reportLabel} — Batch ${d.batchNumber ?? "—"}`;

  const html = emailWrap(
    `${d.reportLabel}: ${d.batchNumber ?? "—"}`,
    `<p style="margin:0 0 16px;color:#555;font-size:14px">
      ${d.reportLabel} report generated for batch ${d.batchNumber ?? "—"}.
    </p>
    ${table([
      ["Batch",        d.batchNumber ?? "—"],
      ["Generated By", d.generatedByName],
