...
export async function POST(req: NextRequest) {
  try {
    ...
    const subject = `[JSCI A-20/1] ${reportLabel} — Batch ${batch?.batch_number ?? "—"}`;
    const html = `
      <p>${reportLabel} report generated for batch ${batch?.batch_number ?? "—"}.</p>
      <p><strong>Generated by:</strong> ${profile?.full_name ?? "—"}</p>
