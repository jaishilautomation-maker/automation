...
export async function POST(req: NextRequest) {
  try {
    ...
    const subject = `[JSCI A-20/1] Crude Sulphur Incoming Report — ${supplier_name || receipt?.supplier_name || batch?.batch_number || "—"}`;
    const html = `
      <p>Crude Sulphur Incoming Inspection report generated.</p>
      <p><strong>Supplier:</strong> ${supplier_name || receipt?.supplier_name || "—"}<br/>
      <strong>Grade:</strong> ${grade ?? "—"}<br/>
      <strong>Generated by:</strong> ${profile?.full_name ?? "—"}</p>
